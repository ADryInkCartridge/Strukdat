...
void itung(BSTNode *root,int money,SinglyList *list)
{
    if (root && root->total<=money)
    {
        if (root->total < money)
        {
            ...
            if (root->left!=NULL )
            {
                root->left->total= root->total + root->left->key;
            }
             if (root->right!=NULL )
            {
                root->right->total= root->total + root->right->key;
            }
        }
        else if (root->total==money)
        {
            slist_pushBack(list,root->key);
            printf("Berhasil!\n");
            SListNode *temp = list->_head;
        ...
        itung(root->left,money,list);
        itung(root->right,money,list);
        slist_popBack(list);
    }
}
...
int main ()
{
    ...
    SinglyList hold;
    slist_init (&hold);
    int testcase,n,in,money;
    freopen("hadu1_in.txt","r",stdin);
	freopen("out.txt","w",stdout);
    scanf ("%d %d",&testcase,&n);
    if (n>100)
    {
        printf("Toko kebanyakan\n");
    }
    else
    {
        for (int i=0;i<n;i++)
